...
If `M` can't find a runnable goroutine from its local run queue, the global run queue, or `netpoll` (will be described in [Network I/O and File I/O](#network-io-and-file-io) section), it keeps spinning for stealing goroutines from other processors `P` and from the global run queue again.
...
Afterward, the main goroutine is created to run the [`runtime.main`](https://github.com/golang/go/blob/go1.24.0/src/runtime/proc.go#L146-L148) function, which serves as the Go runtime entry point.
Within [`runtime.main`](https://github.com/golang/go/blob/go1.24.0/src/runtime/proc.go#L146-L148) function, a dedicated thread is created to launch `sysmon`, which will be described in [Non-cooperative Preemption](#non-cooperative-preemption) section.
...
6. Check netpoll for I/O ready goroutine (described in [Network I/O and File I/O](#network-io-and-file-io) section). 
...
When an instance of [`net.netFD`](https://github.com/golang/go/blob/go1.24.0/src/net/fd_posix.go#L16-L27) is initialized, [`epoll_create`](https://man7.org/linux/man-pages/man2/epoll_create.2.html) system call is invoked to create a **single global** [`epoll`](https://man7.org/linux/man-pages/man7/epoll.7.html) instance, which is used throughout the lifetime of the process.
This is performed in [`poll_runtime_pollServerInit`](https://github.com/golang/go/blob/go1.24.0/src/runtime/netpoll.go#L213-L216) function, and only runs once as being wrapped in a [`sync.Once`](https://pkg.go.dev/sync#Once).
