...
We can see that the `SERIALIZABLE` isolation level has nothing to do with the `INSERT` statement. Therefore, deadlock could definitely happen in the `SERIALIZABLE` isolation level.
