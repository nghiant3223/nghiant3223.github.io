...
In [Figure 1](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md), the method `Prepare` at line 6 tells server to prepare the statement `SELECT * FROM album WHERE id = ?`. Internally, `db.Prepare` will eventually invokes the method `Prepare` of `*mysqlConn`, which resides in package [github.com/go-sql-driver/mysql](https://github.com/go-sql-driver/mysql), as shown in [Figure 2](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md).

```go
 1  package mysql
 2  
 3  func (mc *mysqlConn) Prepare(query string) (driver.Stmt, error) {
 4    ...
 5    // Send command
 6    err := mc.writeCommandPacketStr(comStmtPrepare, query)
 7    ...
 8    // Read Result
 9    columnCount, err := stmt.readPrepareResultPacket()
10    ...
11  }
```
...
```go
 1  package mysql
 2  
 3  func (stmt *mysqlStmt) query(args []driver.Value) (*binaryRows, error) {
 4    ...
 5    // Send command
 6    err := stmt.writeExecutePacket(args)
 7    if err != nil {
 8      return nil, stmt.mc.markBadConn(err)
 9    }
10    ...
11    // Read Result
12    resLen, err := mc.readResultSetHeaderPacket()
13    if err != nil {
14      return nil, err
15    }
16    ...
17    if resLen > 0 {
18      ...
19      rows.rs.columns, err = mc.readColumns(resLen)
20    } else {
21      ...
22    }
23    ...
...
```go
 1  package mysql
 2  
 3  func (stmt *mysqlStmt) Close() error {
 4    ...
 5    err := stmt.mc.writeCommandPacketUint32(comStmtClose, stmt.id)
 6    ...
 7  }
```
...
Line 6 in [Figure 5](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md) tells the server to prepare the statement `SELECT * FROM album WHERE id = ?` and executes the statement with the argument `id`. All of those things happen in the method `db.Query`. Internally, `db.Query` invokes the method `queryDC`, which resides in package `database/sql`, as described in [Figure 6](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md).

```go
 1  package sql
 2  
 3  func (db *DB) queryDC(ctx, txctx context.Context, dc *driverConn, releaseConn func(error), query string, args []any) (*Rows, error) {
 4    ...
 5    withLock(dc, func() {
 6      nvdargs, err = driverArgsConnLocked(dc.ci, nil, args)
 7      if err != nil {
 8        return
 9      }
10      rowsi, err = ctxDriverQuery(ctx, queryerCtx, queryer, query, nvdargs)
11    })
12    if err != driver.ErrSkip {
13      if err != nil {
14        releaseConn(err)
15        return nil, err
16      }
...
20    ...
21    withLock(dc, func() {
22      si, err = ctxDriverPrepare(ctx, dc.ci, query)
23    })
24    ...
25    ds := &driverStmt{Locker: dc, si: si}
26    rowsi, err := rowsiFromStatement(ctx, dc.ci, ds, args...)
27    if err != nil {
28      ds.Close()
29      releaseConn(err)
30      return nil, err
...
33    rows := &Rows{
34      dc:          dc,
35      releaseConn: releaseConn,
36      rowsi:       rowsi,
37      closeStmt:   ds,
38    }
39    rows.initContextClose(ctx, txctx)
40    return rows, nil
41  }
```
...
```go
 1  package mysql
 2  
 3  func (mc *mysqlConn) query(query string, args []driver.Value) (*textRows, error) {
 4    ...
 5    if len(args) != 0 {
 6      if !mc.cfg.InterpolateParams {
 7        return nil, driver.ErrSkip
 8      }
 9      // try client-side prepare to reduce roundtrip
10      prepared, err := mc.interpolateParams(query, args)
11      if err != nil {
12         return nil, err
13       }
14       query = prepared
15     }
16    // Send command
17    err := mc.writeCommandPacketStr(comQuery, query)
18    if err == nil {
19      // Read Result
20      var resLen int
21      resLen, err = mc.readResultSetHeaderPacket()
22      if err == nil {
23        ...
24        // Columns
25        rows.rs.columns, err = mc.readColumns(resLen)
26        return rows, err
27      }
28    }
29    return nil, mc.markBadConn(err)
30  }
```
...
```go
 ...
 4    ...
 5    withLock(dc, func() {
 6      si, err = ctxDriverPrepare(ctx, dc.ci, query)
 7    })
 8    ...
 9    ds := &driverStmt{Locker: dc, si: si}
10    rowsi, err := rowsiFromStatement(ctx, dc.ci, ds, args...)
11    if err != nil {
12      ds.Close()
13      releaseConn(err)
14      return nil, err
...
17    rows := &Rows{
18      dc:          dc,
19      releaseConn: releaseConn,
20      rowsi:       rowsi,
21      closeStmt:   ds,
22    }
23    rows.initContextClose(ctx, txctx)
24    return rows, nil
25  }
```

Afterward, the control is back at line 13 in [Figure 5](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md), where method `Scan` uses the instance of `Rows` returned by `queryDC` to scan an instance of `Album`. At the end of F[igure 5](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md), `rows.Close()` is called, which will invoke the method `Close` of `*myStmt` as [Figure 4](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md).
...
Have you noticed anything abnormal in [Figure 0](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md)? The prepared statement is never closed, meaning that it is never deallocated by the database server. *This is the root cause of the incident.*

When we use explicit prepared statement, it is important to invoke both `stmt.Close` to close the prepared statement and invoke `rows.Close` to return the connection to the pool. In [Figure 0](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md), because `stmt.Close` is not called, the prepared statement is not closed. This is the place where unfortunate events unfold merely due to neglecting a minor method call. 😄
...
In [Figure 0](Prepared%20Statement%20in%20MySQL%20and%20Go%207a81af3563ee4ad69cfcff80ac65fa49.md), neither `MaxConnLifetime` nor `MaxIdleTimeout` is set for any connection in the connection pool `db`, which means the connections never expire. This could result in an unbounded growth in the number of prepared statements if the connections are not closed. 
...
```go
 1  package gorm
 2  
 3  func (db *PreparedStmtDB) prepare(ctx context.Context, conn ConnPool, isTransaction bool, query string) (Stmt, error) {
 4    ...
 5    if stmt, ok := db.Stmts[query]; ok && (!stmt.Transaction || isTransaction) {
 6      ...
 7      return stmt, nil
 8    }
 9    ...
10    stmt, err := conn.PrepareContext(ctx, query)
11    if err == nil {
12      db.Stmts[query] = Stmt{Stmt: stmt, Transaction: isTransaction}
13      db.PreparedSQL = append(db.PreparedSQL, query)
14      ...
15    }
16  
17    return db.Stmts[query], err
18  }
19  
20  func (db *PreparedStmtDB) QueryContext(ctx context.Context, query string, args ...interface{}) (rows *sql.Rows, err error) {
21    stmt, err := db.prepare(ctx, db.ConnPool, false, query)
22    if err == nil {
23      rows, err = stmt.QueryContext(ctx, args...)
24      ...
25    }
26    return rows, err
27  }
28  
29  func (db *PreparedStmtDB) ExecContext(ctx context.Context, query string, args ...interface{}) (result sql.Result, err error) {
30    stmt, err := db.prepare(ctx, db.ConnPool, false, query)
31    if err == nil {
32      result, err = stmt.ExecContext(ctx, args...)
33      ...
34    }
35    return result, err
36  }
37  
38  func (db *PreparedStmtDB) Close() {
39    ...
40    for _, query := range db.PreparedSQL {
41      if stmt, ok := db.Stmts[query]; ok {
42        ...
43        go stmt.Close()
44      }
45    }
